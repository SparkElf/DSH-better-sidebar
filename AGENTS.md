...
- **布局变量**（写在 `<html>` 上，面板打开时有效）：`--dsh-sidebar-width` / `--dsh-sidebar-height`（面板几何；拖拽期间逐帧更新）。右面板宽度作为 AppFrame（新版 `#root [data-dsh-frame]` / rc.8 `#root > [data-slot="root"] > div`）的 `padding-right` 预留，保持 AppFrame border box 为完整桌面视口宽度，避免 Harness 因插件面板展开误入窄屏布局；AppFrame 的 details 拖拽手柄按同一变量向左平移，保持贴合 details 列边缘。底部面板仍通过 centerCol 的 `margin-bottom` 推挤；centerCol 锚点是**复合选择器**（同一元素双保险，禁止退回 `nth-child` 位置锚定）：`#root [data-dsh-frame] > [data-pane="conversation"]` **与** `#root :has(> [data-slot="conversation"])`（0.1.x 命名 / rc.8 时代命名，live 页面实测同元素；`drag-layout.e2e.ts` 断言两者同元素）。
